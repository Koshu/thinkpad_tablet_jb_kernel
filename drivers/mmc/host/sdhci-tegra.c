...
#define DEBOUNCE_TIME	2000
	...
	struct tegra_sdhci_platform_data *sd_plat;
	int card_present = -1;
//--
*/
...
{
	//printk("sd_debounce_timer_func\n");
	int cp = gpio_get_value(sd_plat->cd_gpio);
	if( cp == card_present ){
		//sd_host->card_present =
				//(gpio_get_value(sd_host->cd_gpio) == sd_host->cd_gpio_polarity);
		tasklet_schedule(&sd_host->card_tasklet);
	}
}
//--
*/
...
/*
//++Charles
	card_present = gpio_get_value(sd_plat->cd_gpio);
	
	//printk("carddetect_irq, card_present:%d, cd_gpio:%d\n", card_present, sd_plat->cd_gpio);

	if( sd_plat->cd_gpio == TEGRA_GPIO_PI5 && !card_present){
		card_present = gpio_get_value(sd_plat->cd_gpio);
...
//++Charles
	...
	if (plat->cd_gpio == TEGRA_GPIO_PI5){
		sd_plat = plat;
		sd_host = host;
	}
//--
