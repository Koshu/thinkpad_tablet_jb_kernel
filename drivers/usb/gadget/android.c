...
/* Compal Earvin 20120222 begin */
static void android_notify_work(struct work_struct *data) {
	struct android_dev *dev = container_of(data, struct android_dev, notify_work);
	struct usb_composite_dev *cdev = dev->cdev;
	char *disconnected[2] = { "USB_STATE=DISCONNECTED", NULL };
	char *connected[2]    = { "USB_STATE=CONNECTED", NULL };
	char **uevent_envp = NULL;
	...
	spin_lock_irqsave(&cdev->lock, flags);
	if (!dev->enabled) {
		uevent_envp = dev->notify_connected ? connected : disconnected;
	}
	spin_unlock_irqrestore(&cdev->lock, flags);
	if (uevent_envp) {
		kobject_uevent_env(&dev->dev->kobj, KOBJ_CHANGE, uevent_envp);
...
static void android_notify_connect_state(struct usb_gadget *gadget, int state) {
	struct android_dev *dev = _android_dev;
	struct usb_composite_dev *cdev = get_gadget_data(gadget);
	unsigned long flags;

	spin_lock_irqsave(&cdev->lock, flags);
	dev->notify_connected = state;
	schedule_work(&dev->notify_work);
	spin_unlock_irqrestore(&cdev->lock, flags);
}
/* Compal Earvin 20120222 end */
...
static int __init init(void)
{
	...
	/* Compal Earvin 20120223 begin */
	INIT_WORK(&dev->notify_work, android_notify_work);
	/* Compal Earvin 20120223 end */
	...
	/* Compal Earvin 20120223 begin */
	composite_driver.notify_connect_state = android_notify_connect_state;
	/* Compal Earvin 20120223 end */
