...
{
	struct tegra_fb_info *tegra_fb = info->par;
  printk("fb.c ,tegra_fb_blank(blank=%d)\n",blank);
	switch (blank) {
	...
	case FB_BLANK_NORMAL:
		...
		return 0;
  	/* carry-1201 begin */
	/* [HDMI] add on & off command for /sys/devices/nvhost/tegradc.1/graphics/fb1/blank */
	case FB_BLANK_VSYNC_SUSPEND:
		if(tegra_fb->win->dc->disableHDMI==true){    		
			printk("fb.c ,enable HDMI...\n");
			tegra_fb->win->dc->disableHDMI=false;
			tegra_fb->win->dc->out_ops->detect(tegra_fb->win->dc);
			printk("fb.c ,HDMI already enable!\n");
		}
		return 0;
	case FB_BLANK_HSYNC_SUSPEND:
		if(tegra_fb->win->dc->disableHDMI==false){    		
			printk("fb.c ,disable HDMI...\n");
			tegra_fb->win->dc->disableHDMI=true;
			tegra_fb->win->dc->out_ops->detect(tegra_fb->win->dc);
			printk("fb.c ,HDMI already disable!\n");
		}
		return 0;
  	/* carry-1201 end */
